...
import { Upload, FileSpreadsheet, Calendar, User, AlertCircle, CheckCircle2, Clock, FileText, ArrowRight, Watch, Users, CalendarDays, Activity, ShieldCheck, ChevronLeft, ChevronUp, Fingerprint, Printer, X, Plus, Timer, ArrowDownRight, ArrowUpRight, Download, ChevronDown, Table, Save, LogIn, Pencil } from 'lucide-react';
import ExcelJS from 'exceljs';
import { saveAs } from 'file-saver';
import { Footer } from './components/Footer';
import { db } from './firebase';
import { collection, doc, setDoc, addDoc, updateDoc, deleteDoc, onSnapshot, serverTimestamp, query, getDocs, limit, where } from 'firebase/firestore';
import { getFirebaseInstance } from './lib/databaseManager';
import { useToast } from './components/Toast';
import ZKConnectPanel from './components/ZKConnectPanel';
...
export default function FingerprintAnalysis({ onBack, canEdit = true }: { onBack: () => void; canEdit?: boolean }) {
  ...
  // مزامنة الشيفتات مع Firestore
  useEffect(() => {
    let unsub: (() => void) | undefined;
    try {
      unsub = onSnapshot(collection(dbForSaves, 'shifts'), snap => {
        if (!snap.empty) {
          const fs = snap.docs.map(d => ({ id: d.id, ...d.data() } as Shift));
          setShifts(fs);
          try { localStorage.setItem(SHIFTS_STORAGE_KEY, JSON.stringify(fs)); } catch {}
        }
      }, () => {});
    } catch {}
    return () => unsub?.();
  ...
  const calculateDayStats = (day: any) => {
    ...
    if (selectedShift && !day.isAbsent && day.records.length > 0) {
      ...
      if (day.records.length === 1) {
        ...
        // ── تأخير الدخول ──
        // التأخير = وقت الدخول الفعلي - وقت بداية الشيفت (لو تجاوز السماح)
        if (firstRecord > allowedStart) {
          latenessMinutes = Math.max(0, differenceInMinutes(firstRecord, shiftStart));
        }

        // ── الانصراف المبكر ──
        // الانصراف المبكر = وقت نهاية الشيفت - وقت الخروج الفعلي (لو قبل السماح)
    ...
    if (!day.isAbsent && day.records.length >= 2) {
      const sortedForWork = [...day.records].sort((a: any, b: any) => a.dateTime.getTime() - b.dateTime.getTime());
      const first = sortedForWork[0].dateTime;
      const last  = sortedForWork[sortedForWork.length - 1].dateTime;
      let workMins = differenceInMinutes(last, first);
  ...
  // معالجة بيانات جهاز البصمة المباشر (من ZKConnectPanel)
  const handleZKData = async (rows: { name: string; dateStr: string; timeStr: string }[]) => {
    if (!rows || rows.length === 0) {
      addToast('لم يتم استلام بيانات من جهاز البصمة', 'error');
      return;
    }
    // تحويل صفوف البصمة لنفس format الـ parsedData من Excel
    const parsedData: AttendanceRecord[] = rows
      .map(r => {
        try {
          const dateTime = new Date(`${r.dateStr}T${r.timeStr}`);
          if (isNaN(dateTime.getTime())) return null;
          return { name: r.name, dateStr: r.dateStr, timeStr: r.timeStr, dateTime };
        } catch { return null; }
      })
      .filter(Boolean) as AttendanceRecord[];

    if (parsedData.length === 0) {
      addToast('لم يتم العثور على بيانات صالحة من جهاز البصمة', 'error');
      return;
    }

    // نفس منطق handleFileUpload من سطر set records
    setRecords(parsedData);
    const uniqueNames = new Set(parsedData.map(r => r.name));
    ...
    try {
      ...
      snapshot.docs.forEach(d => {
        ...
        if (data.name) {
          ...
          if (data.fingerprintName) localSortOrder[data.fingerprintName.trim()] = order;
        }
      });
    } catch {}
    ...
    setStep('options');
    addToast(`✅ تم استلام ${parsedData.length} بصمة من جهاز البصمة`, 'success');
  };
  ...
  const openEditShift = (shift: Shift) => {
    ...
    setNewShift({ name: shift.name, startTime: shift.startTime, endTime: shift.endTime, graceIn: shift.graceIn, graceOut: shift.graceOut });
  ...
  const handleSaveShift = async () => {
    ...
    const shiftData = { name: newShift.name!, startTime: newShift.startTime!, endTime: newShift.endTime!, graceIn: newShift.graceIn || 0, graceOut: newShift.graceOut || 0 };
    if (editingShift) {
      try { await setDoc(doc(dbForSaves, 'shifts', editingShift.id), shiftData); } catch {}
      setShifts(prev => prev.map(s => s.id === editingShift.id ? { ...shiftData, id: editingShift.id } : s));
      addToast(`✅ تم تعديل شيفت "${shiftData.name}"`, 'success');
    } else {
      let newId = Date.now().toString();
      try { const ref = await addDoc(collection(dbForSaves, 'shifts'), shiftData); newId = ref.id; } catch {}
      setShifts(prev => [...prev, { ...shiftData, id: newId }]);
      addToast(`✅ تم إضافة شيفت "${shiftData.name}"`, 'success');
    }
  ...
  const handleDeleteShift = async (shiftId: string, shiftName: string) => {
    try { await deleteDoc(doc(dbForSaves, 'shifts', shiftId)); } catch {}
    setShifts(prev => prev.filter(s => s.id !== shiftId));
    addToast(`تم حذف شيفت "${shiftName}"`, 'success');
  };

  const handleAddShift = handleSaveShift;
  ...
  const generateReport = () => {
    ...
    uniqueTargetEmployees.forEach(officialName => {
      ...
      // ── معالجة شيفت الليل: بصمة الخروج بعد منتصف الليل تخص اليوم السابق ──
      // لو السجل وقع بين 00:00 و 06:00 صباحاً، ونفس الموظف عنده سجل في اليوم السابق
      // مباشرة (يعني كان شغّال أصلاً)، فهذا السجل هو "خروج" خاص باليوم السابق، لا بصمة جديدة لليوم الحالي.
      ...
      const NIGHT_CUTOFF_HOUR = 6; // أي بصمة قبل الساعة 6 صباحاً تعتبر "بعد منتصف الليل"
      const adjustedEmpRecords: AttendanceRecord[] = empRecords.map(r => {
        ...
        if (hour < NIGHT_CUTOFF_HOUR) {
          ...
          // هل يوجد سجل للموظف في اليوم السابق؟ (دليل على أنه كان في شيفت ليلي بدأ قبل منتصف الليل)
          const hasPrevDayRecord = (recordsByDateStr[prevDateStr] || []).length > 0;

          if (hasPrevDayRecord) {
            // أعد إسناد هذه البصمة لليوم السابق (خروج الشيفت الليلي)
            return { ...r, dateStr: prevDateStr };
          }
        }
        return r;
      });
          ...
          <div className="bg-[#e6e1d6] rounded-2xl shadow-2xl w-full max-w-md overflow-hidden border border-[#e6dfd3]" dir="rtl">
            <div className="bg-[#2c1e16] px-6 py-4 flex justify-between items-center">
              <h3 className="text-xl font-bold text-white flex items-center gap-2">
                ...
                {editingShift ? `تعديل: ${editingShift.name}` : 'إضافة شيفت جديد'}
              </h3>
              <button 
                onClick={() => { setIsShiftModalOpen(false); setEditingShift(null); setNewShift({ name: '', startTime: '09:00', endTime: '17:00', graceIn: 15, graceOut: 15 }); }}
            ...
            <div className="bg-[#e6dfd3]/50 px-6 py-4 flex justify-end gap-3">
              ...
              <button 
                ...
                className="px-5 py-2.5 rounded-xl font-bold text-white bg-[#76151e] hover:bg-[#5a0f16] transition-colors shadow-md flex items-center gap-2"
              >
                <Save size={16} />
                {editingShift ? 'حفظ التعديلات' : 'إضافة الشيفت'}
              </button>
            </div>
          </div>
        </div>
      ...
      <main className="flex-1 flex flex-col max-w-[1400px] mx-auto w-full px-4 py-4 sm:py-8 z-10 relative">
        {step === 'upload' && (
          <div className="mt-4 sm:mt-10 flex flex-col gap-4">
            ...
            {/* ── زر الاتصال بجهاز البصمة ── */}
            <ZKConnectPanel
              onDataLoaded={(parsedRows) => {
                // نعيد استخدام نفس منطق handleFileUpload لكن مع بيانات جاهزة
                handleZKData(parsedRows);
              }}
            />

            {/* ── رفع ملف Excel (الطريقة التقليدية) ── */}
            <div className="flex flex-col items-center justify-center py-12 sm:py-20 bg-[#fffdfa] rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.08)] border border-[#e6dfd3] relative overflow-hidden group px-4">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                ...
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button 
                    onClick={() => { setEditingShift(null); setNewShift({ name: '', startTime: '09:00', endTime: '17:00', graceIn: 15, graceOut: 15 }); setIsShiftModalOpen(true); }}
                      ...
                      <div key={shift.id} className="bg-white border border-[#e6dfd3] rounded-xl p-4 shadow-sm relative group">
                        <div className="absolute top-2 left-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => openEditShift(shift)}
                            className="w-7 h-7 rounded-lg bg-amber-50 border border-amber-200 text-amber-600 flex items-center justify-center hover:bg-amber-500 hover:text-white transition-all" title="تعديل">
                            <Pencil size={12} />
                          </button>
                          <button onClick={() => handleDeleteShift(shift.id, shift.name)}
                            className="w-7 h-7 rounded-lg bg-red-50 border border-red-100 text-red-500 flex items-center justify-center hover:bg-red-500 hover:text-white transition-all" title="حذف">
                            <X size={13} />
                          </button>
                        </div>
                        <h4 className="font-bold text-[#5a4a3f] mb-2 pl-16">{shift.name}</h4>
                        <div className="text-sm text-[#7a6a5f] space-y-1">
                          <p>الحضور: <span className="font-bold text-[#3a2a1f]">{shift.startTime}</span> (سماح {shift.graceIn} د)</p>
                          <p>الانصراف: <span className="font-bold text-[#3a2a1f]">{shift.endTime}</span> (سماح {shift.graceOut} د)</p>
                        </div>
                      </div>
                    ))}
                  </div>
