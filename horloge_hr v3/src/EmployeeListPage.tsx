...
export default function EmployeeListPage({
  ...
  // ألوان الأفاتار حسب الحرف الأول (v16)
  const avatarColors = ['bg-[#76151e]','bg-[#2563eb]','bg-[#059669]','bg-[#7c3aed]','bg-[#d97706]','bg-[#dc2626]','bg-[#0891b2]','bg-[#65a30d]'];
  const avatarColor = (name: string) => avatarColors[(name?.charCodeAt(0) || 0) % avatarColors.length];

  const empTable = (emps: Employee[], mode: 'active' | 'resigned') => (
    <div className="bg-white/60 backdrop-blur-md rounded-2xl border border-white/60 shadow-sm overflow-hidden">
      <div className="grid grid-cols-12 gap-2 px-5 py-3 bg-[#3a2a1f] text-white text-xs font-black tracking-wide">
        <div className="col-span-1 text-center">#</div>
        <div className="col-span-5">الموظف</div>
        <div className="col-span-3 text-center">الفرع</div>
        <div className="col-span-3 text-center">إجراءات</div>
      </div>
      <div className="divide-y divide-[#e6dfd3]">
        {emps.map((emp, idx) => (
          <div key={emp.id}
            className={`grid grid-cols-12 gap-2 px-5 py-3.5 items-center transition-colors hover:bg-[#76151e]/5
              ${idx % 2 === 0 ? 'bg-white/30' : 'bg-white/10'}`}>
            <div className="col-span-1 flex justify-center">
              <span className="w-7 h-7 rounded-full bg-[#3a2a1f]/8 text-[#7a6a5f] flex items-center justify-center font-black text-xs border border-[#d4c4b7]">
                {emp.sortOrder ?? idx + 1}
              </span>
            </div>
            <div className="col-span-5 flex items-center gap-3 min-w-0">
              <div className={`w-10 h-10 rounded-2xl ${mode === 'resigned' ? 'bg-gray-400' : avatarColor(emp.name || '')} text-white flex items-center justify-center font-black text-lg shadow-md shrink-0`}>
                {emp.name?.charAt(0) ?? '?'}
              </div>
              <div className="min-w-0">
                <p className={`font-black truncate ${mode === 'resigned' ? 'text-gray-500' : 'text-[#3a2a1f]'}`}>{emp.name}</p>
                <div className="flex items-center gap-1.5 mt-0.5">
                  {emp.jobTitle && <span className="text-[10px] font-bold bg-[#76151e]/10 text-[#76151e] px-2 py-0.5 rounded-full truncate max-w-[140px]">{emp.jobTitle}</span>}
                  {mode === 'resigned' && <span className="text-[10px] font-bold bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full">مستقيل</span>}
                </div>
              </div>
            </div>
            <div className="col-span-3 flex justify-center">
              {emp.branch
                ? <span className="text-xs font-bold bg-[#3a2a1f]/8 text-[#3a2a1f] px-3 py-1 rounded-full border border-[#d4c4b7] truncate max-w-full">{emp.branch}</span>
                : <span className="text-[#c4b4a7] text-xs">—</span>}
            </div>
            <div className="col-span-3 flex items-center justify-center gap-1.5">
              <button onClick={() => openEmpModal('view', emp)}
                className="w-8 h-8 rounded-xl bg-blue-50 border border-blue-200 text-blue-600 flex items-center justify-center hover:bg-blue-600 hover:text-white transition-all" title="عرض">
                <Eye size={14}/>
              </button>
              {canEdit && mode === 'active' && <>
                <button onClick={() => openEmpModal('edit', emp)}
                  className="w-8 h-8 rounded-xl bg-amber-50 border border-amber-200 text-amber-600 flex items-center justify-center hover:bg-amber-500 hover:text-white transition-all" title="تعديل">
                  <Pencil size={14}/>
                </button>
                {resignConfirmId === emp.id ? (
                  <div className="flex items-center gap-1">
                    <button onClick={() => openResignModal(emp)} className="text-[9px] bg-orange-500 text-white font-black px-2 py-1 rounded-lg hover:bg-orange-600 transition-all">تأكيد</button>
                    <button onClick={() => setResignConfirmId(null)} className="text-[9px] bg-gray-200 text-gray-600 font-black px-2 py-1 rounded-lg hover:bg-gray-300 transition-all">لا</button>
                  </div>
                ) : (
                  <button onClick={() => setResignConfirmId(emp.id)}
                    className="w-8 h-8 rounded-xl bg-gray-100 border border-gray-200 text-gray-400 hover:bg-orange-50 hover:border-orange-300 hover:text-orange-500 flex items-center justify-center transition-all" title="إنهاء خدمة">
                    <UserX size={14}/>
                  </button>
                )}
                <button onClick={() => setConfirmDelete({ open: true, id: emp.id, name: emp.name })}
                  className="w-8 h-8 rounded-xl bg-red-50 border border-red-200 text-red-400 flex items-center justify-center hover:bg-red-500 hover:text-white transition-all" title="حذف">
                  <Trash2 size={14}/>
                </button>
              </>}
              {canEdit && mode === 'resigned' && (
                <button onClick={() => reactivateEmployee(emp.id)}
                  className="flex items-center gap-1 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-[10px] px-2.5 py-1.5 rounded-xl transition-all">
                  <UserCheck size={12}/>إعادة
                </button>
              )}
            </div>
          </div>
      ...
      <div className="px-5 py-2.5 bg-[#f5f0e8] border-t border-[#e6dfd3] text-xs text-[#9a8a7f] font-bold text-left">
        إجمالي: {emps.length} موظف
      </div>
    </div>
  );
        ...
        {tab === 'active' && (
          <div>
            ...
            {loading ? <div className="flex justify-center py-16"><div className="w-10 h-10 border-4 border-[#76151e] border-t-transparent rounded-full animate-spin"/></div>
              ...
              : empTable(filteredActive, 'active')}
          </div>
        )}
        ...
        {tab === 'resigned' && (
          <div>
            ...
            {filteredResigned.length === 0
              ...
              : empTable(filteredResigned, 'resigned')}
          </div>
        )}
