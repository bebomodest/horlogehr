...
import { ChevronLeft, Plus, Pencil, Trash2, X, Save, Search, Eye, AlertCircle, Filter, FileText } from 'lucide-react';
...
export default function DeductionsPage({ onBack, canEdit = true }: { onBack: () => void; canEdit?: boolean }) {
  ...
  const [rptType,    setRptType]    = useState<'all'|'deduction'|'penalty'|'bonus'>('all');
  const [rptFrom,    setRptFrom]    = useState('');
  const [rptTo,      setRptTo]      = useState('');
  const [rptResults, setRptResults] = useState<typeof records | null>(null);
  ...
  const showReport = () => {
    const results = records.filter(r => {
      if (rptType !== 'all' && r.type !== rptType) return false;
      if (rptEmp && r.employeeName !== rptEmp) return false;
  ...
  const TYPE_LABEL: Record<string,string> = { deduction:'خصم', penalty:'جزاء', bonus:'بدل' };

  const exportReportPDF = async () => {
    if (!rptResults || rptResults.length === 0) return;
    const { buildReportHTML, exportAsPDF } = await import('../lib/exportTemplate');
    const headers = ['الموظف','التاريخ','النوع','الوحدة','المقدار','السبب'];
    const rows = rptResults.map(r => [r.employeeName, r.date, TYPE_LABEL[r.type]||r.type, r.unit==='days'?'أيام':'جنيه', r.amount, r.reason||'-']);
    const subtitle = `${rptEmp||'كل الموظفين'} | ${rptFrom||'—'} → ${rptTo||'—'}`;
    const html = await buildReportHTML('تقرير الخصومات والبدلات', subtitle, 'النظام', 'مسؤول الموارد البشرية', headers, rows, [], 'landscape');
    exportAsPDF(html, 'تقرير-الخصومات.pdf');
  };
      ...
      <div className="w-full flex items-center justify-between p-6 z-10 relative">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => { setRptResults(null); setReportOpen(true); }}
            className="flex items-center gap-2 bg-[#3a2a1f] text-white px-4 py-2.5 rounded-full font-bold shadow-md hover:bg-[#76151e] transition-all text-sm">
            <FileText size={16}/><span>تقارير</span>
          </button>
          {canEdit && <button onClick={openAdd} className="flex items-center gap-2 bg-[#76151e] text-white px-4 py-2.5 rounded-full font-bold shadow-md hover:bg-[#8a1923] transition-all text-sm">
            <Plus size={18}/><span>خصم / جزاء</span>
          </button>}
          {canEdit && (
            <button onClick={() => {
              setModalMode('add');
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setReportOpen(false)}>
          <div className="bg-[#f5f0e8] rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-5 border-b border-[#d4c4b7]">
              <h2 className="text-xl font-black text-[#3a2a1f]">تقرير الخصومات والبدلات</h2>
            ...
            <div className="p-5 flex flex-col gap-4">
              ...
              <div>
                <label className="block text-sm font-black text-[#3a2a1f] mb-1">النوع</label>
                <select value={rptType} onChange={e => { setRptType(e.target.value as any); setRptResults(null); }}
                  className="w-full bg-white/70 border-2 border-[#d4c4b7] focus:border-[#76151e] rounded-xl px-4 py-2.5 font-bold text-[#3a2a1f] focus:outline-none">
                  <option value="all">الكل</option>
                  <option value="deduction">خصومات فقط</option>
                  <option value="penalty">جزاءات فقط</option>
                  <option value="bonus">بدلات فقط</option>
                </select>
              </div>
              ...
              {rptResults !== null && (
                <div className="flex flex-col gap-3">
                  <div className="flex items-center justify-between">
                    <p className="font-black text-[#3a2a1f]">النتائج ({rptResults.length} سجل)</p>
                    {rptResults.length > 0 && (
                  ...
                  {rptResults.length === 0
                    ? <p className="text-center py-6 text-[#a09080] font-bold">لا توجد سجلات</p>
                    : <div className="flex flex-col gap-2 max-h-72 overflow-y-auto">
                        {rptResults.map(r => (
                          <div key={r.id} className="bg-white/60 rounded-xl px-4 py-3 border border-white/60 flex items-center gap-3">
                            <span className={`text-xs font-black px-2 py-1 rounded-full border shrink-0 ${r.type==='bonus'?'text-emerald-600 bg-emerald-50 border-emerald-100':r.type==='penalty'?'text-orange-600 bg-orange-50 border-orange-100':'text-red-600 bg-red-50 border-red-100'}`}>
                              {TYPE_LABEL[r.type]}
                            </span>
                            <div className="flex-1 min-w-0">
                              <p className="font-black text-[#3a2a1f] text-sm truncate">{r.employeeName}</p>
                              <p className="text-xs text-[#7a6a5f]">{r.date} · {r.reason||'—'}</p>
                            </div>
                            <p className="font-black text-[#76151e] shrink-0">{r.amount} {r.unit==='days'?'يوم':'ج'}</p>
                          </div>
                        ))}
                      </div>
                  }
