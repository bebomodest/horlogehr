...
// تحويل الوقت من نظام 24 ساعة إلى 12 ساعة مع صباحاً/مساءً
...
export default function ReportsView({ onBack }: { onBack: () => void }) {
  ...
  const [employees, setEmployees] = useState<{ name: string; annualLeaveBalance: number; status?: string; resignedDate?: string }[]>([]);
  ...
  // إجمالي أيام الإجازة المستخدمة لكل موظف (من كل السجلات بغض النظر عن الفترة)
  const [allAnnualUsed, setAllAnnualUsed] = useState<Record<string, number>>({});

  useEffect(() => {
    const parseEmps = (snap: any) =>
      snap.docs
        .map((d: any) => ({
          name: d.data().name as string,
          sortOrder: (d.data().sortOrder ?? 9999) as number,
          status: d.data().status,
          annualLeaveBalance: (d.data().annualLeaveBalance ?? 0) as number,
          resignedDate: (d.data().resignedDate || '') as string,
        }))
        .filter((e: any) => e.name && (e.status !== 'resigned' || (e.resignedDate && e.resignedDate.trim() !== '')))
        .sort((a: any, b: any) => a.sortOrder - b.sortOrder || a.name.localeCompare(b.name, 'ar'))
        .map((e: any) => ({ name: e.name, annualLeaveBalance: e.annualLeaveBalance, status: e.status, resignedDate: e.resignedDate }));

    getDocs(collection(dbForEmps, 'employees')).then(snap => {
      setEmployees(parseEmps(snap));
    }).catch(() => {
  ...
  // حساب ساعات العمل الفعلية بدقة مع دعم الشيفت الليلي
  const calculateWorkMinutes = (r: FingerprintRecord): number => {
    if (!r.logs || r.logs.length < 2) return 0;
    // تحويل الأوقات لدقائق ثم ترتيب
    const toMins = (t: string) => {
      const [h, m] = t.split(':').map(Number);
      return h * 60 + m;
    };
    const times = r.logs.map(l => toMins(l.time)).filter(m => !isNaN(m));
    if (times.length < 2) return 0;
    times.sort((a, b) => a - b);
    const first = times[0];
    const last = times[times.length - 1];
    let mins = last - first;
    // لو الفرق سالب أو صغير جداً (شيفت ليلي عبر منتصف الليل)
    if (mins < 0) mins += 24 * 60;
    // لو أكبر من 16 ساعة، على الأرجح شيفت ليلي مع بصمات في منتصف الليل
    // نستخدم الفرق الأصغر
    if (mins > 16 * 60) {
      const altMins = 24 * 60 - mins;
      mins = altMins;
    }
    return mins;
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // ── فلترة الموظفين المستقيلين بتاريخ الاستقالة ──────────────────────────
      // الموظف المستقيل يظهر فقط في التقارير حتى تاريخ استقالته
      const resignMap: Record<string, string> = {};
      employees.forEach(e => { if (e.status === 'resigned' && e.resignedDate) resignMap[e.name] = e.resignedDate; });
      // بعد فلترة السجلات — أزل السجلات التي تتجاوز تاريخ الاستقالة
      records = records.filter(r => {
        const rDate = resignMap[r.employeeName];
        if (rDate && r.date > rDate) return false;
        return true;
      });
      ...
      // رتّب النتائج بنفس ترتيب صفحة قائمة الموظفين
      const orderIndex: Record<string, number> = {};
      employees.forEach((emp, idx) => { orderIndex[emp.name] = idx; });
      records.sort((a, b) => {
      ...
      // ── حساب إجمالي الإجازات المستخدمة من كل السجلات ─────────────────────
      // (الرصيد الكلي = annualLeaveBalance + totalUsed للحصول على الرصيد الأصلي)
      try {
        const allSnap = await getDocs(query(collection(dbToUse, 'fingerprints')));
        const usedMap: Record<string, number> = {};
        allSnap.docs.forEach(d => {
          if (d.data().status === 'annual') {
            const name = d.data().employeeName || '';
            if (name) usedMap[name] = (usedMap[name] || 0) + 1;
          }
        });
        setAllAnnualUsed(usedMap);
      } catch { /* ignore */ }

      // ── فلترة نتائج التقارير: المستقيلون يظهرون فقط حتى تاريخ استقالتهم ──
      // وإذا تم اختيار موظفين بعينهم، لا تظهر نتائج للمستقيلين غير المختارين
      const activeEmpNames = new Set(employees.filter(e => e.status !== 'resigned').map(e => e.name));
      const resignedWithDate = new Set(employees.filter(e => e.status === 'resigned' && e.resignedDate).map(e => e.name));

      // في التقرير المجمع: اعرض فقط الموظفين المختارين (الناشطين)
      if (selectedReportType === 'summary') {
        const chosen = selectAll ? activeEmpNames : new Set(selectedEmployees.filter(n => activeEmpNames.has(n)));
        records = records.filter(r => chosen.has(r.employeeName));
      }

      setReportResults(records);
      setShowResults(true);
      if (selectedReportType === 'fingerprint') setFingerprintModalOpen(true);
  ...
  const buildExportData = () => {
    ...
    const tableHeaders: Record<string, string[]> = {
      fingerprint: ['الموظف', 'التاريخ', 'اليوم', 'البصمات / الحالة', 'ساعات العمل', 'الشيفت'],
    ...
    const rows = reportResults.map(r => {
      ...
      if (selectedReportType === 'fingerprint') {
        ...
        const cell = sl ? sl.text : (r.logs.length === 0 ? 'لا توجد بصمات' : r.logs.map(l=>formatTime12(l.time)).join(' | '));
        const workMins = calculateWorkMinutes(r);
        return [r.employeeName, r.date, day, cell, workMins > 0 ? formatMins(workMins) : '-', r.shiftInfo?.name || '—'];
      } else if (selectedReportType === 'delays') {
        const { lateness, earlyDeparture } = calculateDelays(r);
        const sorted = [...r.logs].sort((a,b)=>a.time.localeCompare(b.time));
        return [r.employeeName, r.date, day, formatMins(lateness), formatMins(earlyDeparture), formatTime12(sorted[0]?.time||'-'), formatTime12(sorted[sorted.length-1]?.time||'-')];
      } else {
        return [r.employeeName, r.date, day, r.status === 'paid' ? 'راحة أسبوعية' : 'إجازة سنوية'];
      }
    });

    // Stats for export (v17/v18: annual leave balance)
    ...
    Object.entries(groupedByEmployee).forEach(([name, recs]) => {
      ...
      } else if (selectedReportType === 'fingerprint') {
        const totalMins = recs.reduce((s, r) => s + calculateWorkMinutes(r), 0);
        statsRows.push({ label: `${name} - إجمالي ساعات العمل`, value: formatMins(totalMins) });
      } else if (selectedReportType === 'annual-leave') {
        const empData = employees.find(e => e.name === name);
        const annualBalance = empData?.annualLeaveBalance ?? 0;
        const usedDays = reportResults.filter(r => r.employeeName === name && r.status === 'annual').length;
        const remaining = annualBalance - usedDays;
        statsRows.push({ label: `${name} — رصيد الإجازات السنوي`, value: `${annualBalance} يوم` });
        statsRows.push({ label: `${name} — الرصيد المستخدم`, value: `${usedDays} يوم` });
        statsRows.push({ label: `${name} — الرصيد المتبقي`, value: `${remaining < 0 ? '−' + Math.abs(remaining) : remaining} يوم` });
      }
    });
  ...
  // v12: sectioned export عند اختيار أكثر من موظف
  const shouldUseSections = selectedReportType !== 'summary' && (selectAll || selectedEmployees.length > 1);

  const buildSectionedExportData = () => {
    const empLabel = selectAll ? 'كل الموظفين' : selectedEmployees.join('، ');
    const subtitle = `${empLabel}  |  ${startDate} - ${endDate}`;
    const titles: Record<string, string> = {
      fingerprint: 'تقرير بصمة', delays: 'تقرير التأخيرات',
    ...
    const sectionHeaders: Record<string, string[]> = {
      fingerprint: ['التاريخ', 'اليوم', 'البصمات / الحالة', 'ساعات العمل', 'الشيفت'],
      delays: ['التاريخ', 'اليوم', 'تأخير دخول', 'خروج مبكر', 'أول دخول', 'آخر خروج'],
      'weekly-rest': ['التاريخ', 'اليوم', 'الحالة'],
      'annual-leave': ['التاريخ', 'اليوم', 'الحالة'],
    };

    const sections = Object.entries(groupedByEmployee).map(([empName, recs]) => {
      let totalWorkMins = 0, totalLateness = 0, totalEarly = 0;
      const rows = recs.map(r => {
        const day = ARABIC_DAYS[new Date(r.date+'T00:00:00').getDay()];
        if (selectedReportType === 'fingerprint') {
          const sl = statusLabel(r.status);
          const cell = sl ? sl.text : (r.logs.length === 0 ? 'لا توجد بصمات' : r.logs.map(l=>formatTime12(l.time)).join(' | '));
          const wm = calculateWorkMinutes(r); totalWorkMins += wm;
          return [r.date, day, cell, wm > 0 ? formatMins(wm) : '-', r.shiftInfo?.name || '—'];
        } else if (selectedReportType === 'delays') {
          const { lateness, earlyDeparture } = calculateDelays(r);
          totalLateness += lateness; totalEarly += earlyDeparture;
          const sorted = [...r.logs].sort((a,b)=>a.time.localeCompare(b.time));
          return [r.date, day, formatMins(lateness), formatMins(earlyDeparture), formatTime12(sorted[0]?.time||'-'), formatTime12(sorted[sorted.length-1]?.time||'-')];
        } else {
          return [r.date, day, r.status === 'paid' ? 'راحة أسبوعية' : 'إجازة سنوية'];
        }
      });

      const statsRows: { label: string; value: string }[] = [];
      if (selectedReportType === 'fingerprint') {
        statsRows.push({ label: 'إجمالي ساعات العمل', value: formatMins(totalWorkMins) });
        statsRows.push({ label: 'أيام بصمة', value: String(recs.filter(r => r.logs.length > 0).length) });
        statsRows.push({ label: 'بدون بصمة', value: String(recs.filter(r => r.logs.length === 0 && !r.status?.match(/paid|annual/)).length) });
      } else if (selectedReportType === 'delays') {
        statsRows.push({ label: 'إجمالي التأخير', value: formatMins(totalLateness) });
        statsRows.push({ label: 'إجمالي الانصراف المبكر', value: formatMins(totalEarly) });
      } else if (selectedReportType === 'annual-leave') {
        const empData = employees.find(e => e.name === empName);
        const annualBalance = empData?.annualLeaveBalance ?? 0;
        const usedDays = recs.filter(r => r.status === 'annual').length;
        const remaining = annualBalance - usedDays;
        statsRows.push({ label: 'رصيد الإجازات السنوي', value: `${annualBalance} يوم` });
        statsRows.push({ label: 'الرصيد المستخدم', value: `${usedDays} يوم` });
        statsRows.push({ label: 'الرصيد المتبقي', value: `${remaining < 0 ? '−' + Math.abs(remaining) : remaining} يوم` });
      }
      return { employeeName: empName, rows, statsRows };
    });

    return { title: titles[selectedReportType!] || 'تقرير', subtitle, headers: sectionHeaders[selectedReportType!] || [], sections };
  };

  const exportPDF = async () => {
    if (shouldUseSections) {
      const { buildSectionedReportHTML, exportAsPDF } = await import('../lib/exportTemplate');
      const { title, subtitle, headers, sections } = buildSectionedExportData();
      const html = await buildSectionedReportHTML(title, subtitle, 'النظام', 'مسؤول الموارد البشرية', headers, sections, 'landscape');
  ...
  const exportExcel = async () => {
    ...
    if (shouldUseSections) {
      const { exportSectionedExcel } = await import('../lib/exportTemplate');
      const { title, subtitle, headers, sections } = buildSectionedExportData();
      await exportSectionedExcel(ExcelJS, title, subtitle, 'النظام', 'مسؤول الموارد البشرية', headers, sections, `${title}.xlsx`);
      return;
    }
  ...
  return (
    <div dir="rtl" className="flex-1 flex flex-col bg-[#e6e1d6] font-sans bg-[url('https://www.transparenttextures.com/patterns/natural-paper.png')] bg-repeat">
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-6 z-10 relative pb-12">
        ...
        {selectedReportType && (
          <>
            ...
            <div className="bg-white/60 backdrop-blur-md rounded-3xl p-6 mb-4 border border-white/60 shadow-md">
              ...
              {empDropdownOpen && ReactDOM.createPortal(
                <div
                  style={{
                ...
                >
                  <div
                    style={{
                      backgroundColor: '#f5f0e8', borderRadius: '24px',
                  ...
                  >
                    {/* Header */}
                    <div style={{ background: '#3a2a1f', padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        <Users size={18} color="rgba(255,255,255,0.8)"/>
                        <span style={{ color: 'white', fontWeight: 900, fontSize: '18px' }}>اختر الموظفين</span>
                      </div>
                      <button onClick={() => setEmpDropdownOpen(false)}
                        style={{ background: 'rgba(255,255,255,0.15)', border: 'none', borderRadius: '50%', width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'white', fontSize: '16px' }}>
                        ✕
                      </button>
                    </div>
                    {/* Scrollable list */}
                    <div style={{ overflowY: 'auto', flex: 1 }}>
                      ...
                      <div onClick={handleSelectAll}
                        style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 20px', cursor: 'pointer', borderBottom: '1px solid #e6dfd3', background: selectAll ? 'rgba(118,21,30,0.06)' : 'transparent' }}>
                        <div style={{ width: '24px', height: '24px', borderRadius: '8px', border: `2px solid ${selectAll ? '#76151e' : '#d4c4b7'}`, background: selectAll ? '#76151e' : 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                          {selectAll && <span style={{ color: 'white', fontSize: '12px', fontWeight: 900 }}>✓</span>}
                        </div>
                        <span style={{ fontWeight: 900, color: '#76151e', fontSize: '15px', flex: 1 }}>كل الموظفين</span>
                        <span style={{ fontSize: '11px', color: '#7a6a5f', fontWeight: 700, background: 'white', padding: '2px 8px', borderRadius: '20px', border: '1px solid #e6dfd3' }}>
                          {employees.filter(e => e.status !== 'resigned').length} موظف
                        </span>
                      </div>
                      {/* Active employees */}
                      {employees.filter(e => e.status !== 'resigned').map((emp, idx) => {
                        const isSelected = !selectAll && selectedEmployees.includes(emp.name);
                        return (
                          <div key={emp.name} onClick={() => toggleEmployee(emp.name)}
                            style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 20px', cursor: 'pointer', borderBottom: '1px solid #e6dfd3', background: isSelected ? 'rgba(118,21,30,0.05)' : idx % 2 === 0 ? 'white' : 'rgba(250,247,243,0.8)' }}>
                            <div style={{ width: '24px', height: '24px', borderRadius: '8px', border: `2px solid ${isSelected ? '#76151e' : '#d4c4b7'}`, background: isSelected ? '#76151e' : 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                              {isSelected && <span style={{ color: 'white', fontSize: '11px', fontWeight: 900 }}>✓</span>}
                            </div>
                            <div style={{ width: '34px', height: '34px', borderRadius: '50%', background: 'rgba(118,21,30,0.1)', color: '#76151e', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, fontSize: '14px', flexShrink: 0 }}>
                              {emp.name?.charAt(0) ?? '?'}
                            </div>
                            <span style={{ fontWeight: 700, color: '#3a2a1f', flex: 1, fontSize: '14px' }}>{emp.name}</span>
                            {isSelected && <span style={{ fontSize: '10px', background: '#76151e', color: 'white', fontWeight: 900, padding: '2px 8px', borderRadius: '20px' }}>محدد</span>}
                          </div>
                        );
                      })}
                    </div>
                    {/* Footer */}
                    <div style={{ padding: '16px 20px', borderTop: '1px solid #d6cfc3', flexShrink: 0 }}>
                      <button onClick={() => setEmpDropdownOpen(false)}
                        style={{ width: '100%', background: '#76151e', color: 'white', border: 'none', borderRadius: '16px', padding: '12px', fontWeight: 900, fontSize: '15px', cursor: 'pointer' }}>
        ...
        {showResults && (
          <div>
            ...
            {/* OTHER REPORTS — non-fingerprint types show inline */}
            {selectedReportType !== 'summary' && selectedReportType !== 'fingerprint' && Object.entries(groupedByEmployee).map(([empName, records]) => {
                                  ...
                                  <td className="px-4 py-3 text-center text-sm">
                                    {sl
                                      ...
                                      : r.logs.length === 0
                                        ...
                                        : <span className="font-mono">{r.logs.map(l=>formatTime12(l.time)).join('  |  ')}</span>
                                    }
                                  </td>
                                  <td className="px-4 py-3 text-center font-bold text-[#3a2a1f]">
                                    {(() => { const m = calculateWorkMinutes(r); return m > 0 ? formatMins(m) : '-'; })()}
                                  </td>
                                  <td className="px-4 py-3 text-center text-sm">
                                    {r.shiftInfo
                    ...
                    {selectedReportType === 'fingerprint' && (
                      <>
                        ...
                        <div className="bg-[#76151e]/5 border border-[#76151e]/10 rounded-xl p-3 text-center">
                          ...
                          <p className="text-xl font-black text-[#76151e]">{formatMins(records.reduce((s, r) => s + calculateWorkMinutes(r), 0))}</p>
                        </div>
                      </>
                    )}
                    ...
                    {(selectedReportType === 'weekly-rest' || selectedReportType === 'annual-leave') && (
                      <>
                        <div className="bg-purple-50 border border-purple-100 rounded-xl p-3 text-center">
                          <p className="text-purple-700 text-xs font-bold mb-1">أيام مسجلة</p>
                          <p className="text-xl font-black text-purple-600">{records.length} يوم</p>
                        </div>
                        {selectedReportType === 'annual-leave' && (() => {
                          const empData = employees.find(e => e.name === empName);
                          const remaining  = empData?.annualLeaveBalance ?? 0;
                          const totalUsed  = allAnnualUsed[empName] ?? 0;
                          const totalBalance = remaining + totalUsed;
                          return (<>
                            <div className="bg-blue-50 border border-blue-100 rounded-xl p-3 text-center">
                              <p className="text-blue-700 text-xs font-bold mb-1">رصيد الإجازات السنوي</p>
                              <p className="text-xl font-black text-blue-600">{totalBalance} يوم</p>
                            </div>
                            <div className="bg-amber-50 border border-amber-100 rounded-xl p-3 text-center">
                              <p className="text-amber-700 text-xs font-bold mb-1">الرصيد المستخدم</p>
                              <p className="text-xl font-black text-amber-600">{totalUsed} يوم</p>
                            </div>
                            <div className={`${remaining < 0 ? 'bg-red-50 border-red-100' : 'bg-emerald-50 border-emerald-100'} border rounded-xl p-3 text-center`}>
                              <p className={`${remaining < 0 ? 'text-red-700' : 'text-emerald-700'} text-xs font-bold mb-1`}>الرصيد المتبقي</p>
                              <p className={`text-xl font-black ${remaining < 0 ? 'text-red-600' : 'text-emerald-600'}`}>
                                {remaining < 0 ? `−${Math.abs(remaining)}` : remaining} يوم
                              </p>
                            </div>
                          </>);
                        })()}
      ...
      {/* ── FINGERPRINT REPORT MODAL ─────────────────────────────────────── */}
      {fingerprintModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4"
          onClick={() => setFingerprintModalOpen(false)}>
          <div className="bg-[#e6e1d6] rounded-3xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col border border-white/40"
            onClick={e => e.stopPropagation()}>

            {/* Modal Header */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#d4c4b7] bg-[#3a2a1f] rounded-t-3xl shrink-0">
              <div className="flex items-center gap-3">
                <FileText size={20} className="text-white/80"/>
                <h2 className="text-xl font-black text-white">تقرير بصمة</h2>
                <span className="text-sm text-white/60 font-bold">{startDate} — {endDate}</span>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={exportPDF} className="flex items-center gap-2 bg-white/10 hover:bg-white/20 text-white px-3 py-2 rounded-xl font-bold text-sm transition-all">
                  <FileDown size={14}/>PDF
                </button>
                <button onClick={exportExcel} className="flex items-center gap-2 bg-white/10 hover:bg-white/20 text-white px-3 py-2 rounded-xl font-bold text-sm transition-all">
                  <FileDown size={14}/>Excel
                </button>
                <button onClick={handlePrint} className="flex items-center gap-2 bg-white/10 hover:bg-white/20 text-white px-3 py-2 rounded-xl font-bold text-sm transition-all">
                  <Printer size={14}/>طباعة
                </button>
                <button onClick={() => setFingerprintModalOpen(false)}
            ...
            <div className="overflow-y-auto flex-1 p-5">
              {Object.entries(groupedByEmployee).map(([empName, records]) => {
                let totalLateness = 0, totalEarly = 0;
                records.forEach(r => { const d = calculateDelays(r); totalLateness += d.lateness; totalEarly += d.earlyDeparture; });
                return (
                  <div key={empName} className="mb-6">
                    ...
                    <div className="bg-white/60 backdrop-blur-md rounded-2xl border border-white/60 shadow-md overflow-x-auto mb-3">
                      <table className="w-full text-sm" dir="rtl">
                        <thead>
                          <tr className="bg-[#3a2a1f] text-white">
                        ...
                        <tbody>
                          {records.map((r, i) => {
                            ...
                            const workMins = calculateWorkMinutes(r);
                            return (
                              <tr key={r.id} className={i % 2 === 0 ? 'bg-white/30' : 'bg-white/10'}>
                                <td className="px-4 py-3 font-bold text-[#3a2a1f]">{r.date}</td>
                                <td className="px-4 py-3 text-center text-[#5a4a3f] font-bold">
                                  {ARABIC_DAYS[new Date(r.date + 'T00:00:00').getDay()]}
                                </td>
                                <td className="px-4 py-3 text-center text-sm">
                                  {sl ? <span className={`font-bold ${sl.color}`}>{sl.text}</span>
                                     : r.logs.length === 0
                                       ? <span className="text-red-500 font-bold">لا توجد بصمات</span>
                                       : <span className="font-mono text-xs">{r.logs.map(l => formatTime12(l.time)).join('  |  ')}</span>
                                  }
                                </td>
                                <td className="px-4 py-3 text-center font-bold text-[#3a2a1f]">
                                  {workMins > 0 ? formatMins(workMins) : '—'}
                                </td>
            ...
            {/* Modal Footer */}
            <div className="px-6 py-4 border-t border-[#d4c4b7] flex items-center justify-between shrink-0">
              <span className="text-sm text-[#7a6a5f] font-bold">
                {Object.keys(groupedByEmployee).length} موظف · {reportResults.length} سجل
              </span>
              <button onClick={() => setFingerprintModalOpen(false)}
                className="bg-[#3a2a1f] hover:bg-[#76151e] text-white px-6 py-2.5 rounded-2xl font-black transition-all">
                إغلاق
              </button>
            </div>
          </div>
