...
import { ChevronLeft, Eye, Pencil, Shield, Save, X, AlertTriangle, CheckCircle, CheckCircle2, Clock, Search, Upload, User, FileImage, Download, FileDown, Printer, FileSpreadsheet, Trash2 } from 'lucide-react';
...
export default function HealthCertificatesPage({ onBack }: { onBack: () => void }) {
  ...
  const [statusFilter, setStatusFilter] = useState<'all' | 'valid' | 'alert'>('all');
  ...
  const buildExportData = () => {
    ...
    const exportExpired = employees.filter(e => { const d = getDaysRemaining(certificates[e.id]?.expiryDate); return d !== null && d < 0; }).length;
    const exportSoon    = employees.filter(e => { const d = getDaysRemaining(certificates[e.id]?.expiryDate); return d !== null && d >= 0 && d <= 30; }).length;
    const exportValid   = employees.filter(e => { const d = getDaysRemaining(certificates[e.id]?.expiryDate); return d !== null && d > 30; }).length;
    const exportNoCert  = employees.filter(e => !certificates[e.id]).length;
    const statsRows = [
      { label: 'إجمالي الموظفين',        value: String(employees.length) },
      { label: 'لديهم شهادات مسجَّلة',    value: String(registeredCount) },
      { label: 'شهادات سارية (>30 يوم)', value: String(exportValid) },
      { label: 'تنتهي خلال 30 يوم',      value: String(exportSoon) },
      { label: 'شهادات منتهية',           value: String(exportExpired) },
      { label: 'لم يُسجَّل لهم بيانات',    value: String(exportNoCert) },
    ];
  ...
  const filtered = employees.filter(e => {
    if (searchQuery && !e.name.includes(searchQuery) && !e.jobTitle.includes(searchQuery)) return false;
    if (branchFilter !== 'all' && (e as any).branch !== branchFilter) return false;
    if (statusFilter === 'valid') {
      const cert = certificates[e.id];
      if (!cert) return false;
      if (cert.hasCertificate === false) return true;
      const days = getDaysRemaining(cert.expiryDate);
    ...
    if (statusFilter === 'alert') {
      const cert = certificates[e.id];
      if (!cert) return true;
      if (cert.hasCertificate === false) return false;
      const days = getDaysRemaining(cert.expiryDate);
      return days === null || days <= 30;
    }
    return true;
  ...
  // إجمالي الموظفين المسجَّل لهم بيانات في الشهادات
  const registeredCount = employees.filter(e => certificates[e.id] !== undefined).length;

  // منتهية أو تنتهي قريباً = لم يُسجَّل له بيانات OR تاريخ انتهاء ≤ 30 يوم
  ...
  // 🟢 أخضر: مسجَّل وسارية (>30 يوم) أو hasCertificate=false
  // 🔴 أحمر: لم يُسجَّل أو تاريخ انتهاء ≤30 يوم أو بدون تاريخ
  const getRowBg = (empId: string) => {
    const cert = certificates[empId];
    if (!cert) return 'bg-red-50 border-r-4 border-red-400';
    if (cert.hasCertificate === false) return 'bg-emerald-50 border-r-4 border-emerald-400';
    const days = getDaysRemaining(cert.expiryDate);
    if (days === null) return 'bg-red-50 border-r-4 border-red-400';
    if (days <= 30)    return 'bg-red-50 border-r-4 border-red-400';
    return 'bg-emerald-50 border-r-4 border-emerald-400';
  };
  ...
  return (
    <div dir="rtl" className="flex-1 flex flex-col bg-[#e6e1d6] font-sans bg-[url('https://www.transparenttextures.com/patterns/natural-paper.png')]">
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-4 sm:px-6 z-10 relative pb-12">
        ...
        {/* Summary cards — قابلة للنقر كفلاتر */}
        <div className="grid grid-cols-3 gap-3 mb-6">

          {/* كل الموظفين */}
          <button onClick={() => setStatusFilter('all')}
            className={`rounded-2xl p-4 border-2 text-center transition-all hover:scale-[1.02] active:scale-[0.98] shadow-sm
              ${statusFilter === 'all' ? 'bg-[#3a2a1f] border-[#3a2a1f] text-white shadow-lg' : 'bg-white/60 backdrop-blur-md border-white/60 hover:border-[#3a2a1f]/30'}`}>
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mx-auto mb-2 ${statusFilter === 'all' ? 'bg-white/20' : 'bg-[#3a2a1f]/10'}`}>
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" className={statusFilter === 'all' ? 'text-white' : 'text-[#3a2a1f]'}>
                <circle cx="9" cy="7" r="3" stroke="currentColor" strokeWidth="2"/>
                <circle cx="16" cy="7" r="2.5" stroke="currentColor" strokeWidth="1.8"/>
                <path d="M2 19c0-3.314 3.134-6 7-6s7 2.686 7 6" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
                <path d="M17 13c2.21 0 4 1.567 4 3.5" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"/>
              </svg>
            </div>
            <p className={`text-3xl font-black mb-0.5 ${statusFilter === 'all' ? 'text-white' : 'text-[#3a2a1f]'}`}>{employees.length}</p>
            <p className={`text-xs font-bold ${statusFilter === 'all' ? 'text-white/80' : 'text-[#7a6a5f]'}`}>إجمالي الموظفين</p>
            {statusFilter === 'all' && <span className="mt-1.5 inline-block text-[9px] bg-white/20 text-white font-black px-2 py-0.5 rounded-full">معروض الآن</span>}
          </button>

          {/* لديهم شهادة سارية */}
          <button onClick={() => setStatusFilter('valid')}
            className={`rounded-2xl p-4 border-2 text-center transition-all hover:scale-[1.02] active:scale-[0.98] shadow-sm
              ${statusFilter === 'valid' ? 'bg-emerald-600 border-emerald-600 text-white shadow-lg' : 'bg-emerald-50/80 backdrop-blur-md border-emerald-200 hover:border-emerald-400'}`}>
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mx-auto mb-2 ${statusFilter === 'valid' ? 'bg-white/20' : 'bg-emerald-100'}`}>
              <CheckCircle2 size={26} className={statusFilter === 'valid' ? 'text-white' : 'text-emerald-600'} />
            </div>
            <p className={`text-3xl font-black mb-0.5 ${statusFilter === 'valid' ? 'text-white' : 'text-emerald-600'}`}>{registeredCount}</p>
            <p className={`text-xs font-bold ${statusFilter === 'valid' ? 'text-white/80' : 'text-emerald-700'}`}>لديهم شهادة</p>
            {statusFilter === 'valid' && <span className="mt-1.5 inline-block text-[9px] bg-white/20 text-white font-black px-2 py-0.5 rounded-full">معروض الآن</span>}
          </button>

          {/* منتهية / تنتهي قريباً */}
          <button onClick={() => setStatusFilter('alert')}
            className={`rounded-2xl p-4 border-2 text-center transition-all hover:scale-[1.02] active:scale-[0.98] shadow-sm
              ${statusFilter === 'alert' ? 'bg-red-600 border-red-600 text-white shadow-lg' : 'bg-red-50/80 backdrop-blur-md border-red-200 hover:border-red-400'}`}>
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mx-auto mb-2 ${statusFilter === 'alert' ? 'bg-white/20' : 'bg-red-100'}`}>
              <AlertTriangle size={26} className={statusFilter === 'alert' ? 'text-white' : 'text-red-600'} />
            </div>
            <p className={`text-3xl font-black mb-0.5 ${statusFilter === 'alert' ? 'text-white' : 'text-red-600'}`}>{alertCount}</p>
            <p className={`text-xs font-bold ${statusFilter === 'alert' ? 'text-white/80' : 'text-red-700'}`}>منتهية / تنتهي قريباً</p>
            {statusFilter === 'alert' && <span className="mt-1.5 inline-block text-[9px] bg-white/20 text-white font-black px-2 py-0.5 rounded-full">معروض الآن</span>}
          </button>
        ...
        <div className="flex items-center gap-4 mb-4 flex-wrap">
          ...
          <span className="flex items-center gap-1.5 text-xs font-bold text-emerald-700">
            <span className="w-3 h-3 rounded-sm bg-emerald-50 border-r-2 border-emerald-400 inline-block"/>
            شهادة سارية (أكثر من 30 يوم) / ليس له شهادة
          </span>
          <span className="flex items-center gap-1.5 text-xs font-bold text-red-600">
            <span className="w-3 h-3 rounded-sm bg-red-50 border-r-2 border-red-400 inline-block"/>
            لم يُسجَّل / شهادة منتهية أو تنتهي خلال 30 يوم
          </span>
        </div>
