...
const getFileIcon = (type?: string) => {
  ...
  if (t.includes('pdf'))   return <FileText size={22} className="text-red-500"/>;
  if (t.includes('word') || t.includes('doc')) return <FileText size={22} className="text-blue-500"/>;
  if (t.includes('sheet') || t.includes('excel') || t.includes('xls')) return <FileSpreadsheet size={22} className="text-emerald-600"/>;
  if (t.includes('image')) return <Image size={22} className="text-purple-500"/>;
...
const getFileBadgeColor = (type?: string) => {
  ...
  if (t.includes('sheet') || t.includes('excel') || t.includes('xls')) return 'bg-emerald-100 text-emerald-700';
  if (t.includes('image')) return 'bg-purple-100 text-purple-700';
...
const getTypeLabel = (type?: string) => {
  ...
  if (t.includes('sheet') || t.includes('excel') || t.includes('xls')) return 'Excel';
...
export default function HRDocumentsPage({ onBack }: { onBack: () => void }) {
  ...
  // Storage دائماً من المشروع الافتراضي (Firebase Storage لا يتغير بتغيير الـ Firestore database)
...
<script>window.onload = () => { window.print(); }</script>
  ...
  const handleUpload = async () => {
    ...
    try {
        ...
        task.on('state_changed',
          ...
          (err: any) => {
            if (err.code === 'storage/unauthorized') {
              reject(new Error('ليس لديك صلاحية للرفع — تأكد من Firebase Storage Rules'));
            } else if (err.code === 'storage/canceled') {
              reject(new Error('تم إلغاء الرفع'));
            } else {
              reject(new Error(err.message || 'خطأ في الرفع'));
            }
          },
          () => resolve()
        );
      ...
      setUploadError('خطأ: ' + (e.message || 'تحقق من الاتصال وصلاحيات Firebase Storage'));
    } finally {
  ...
  const handleDelete = async (file: HRFile) => {
    ...
    try {
      // حذف من Storage
      try { await deleteObject(ref(storageToUse, file.storagePath)); } catch {}
      // حذف من Firestore
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => !isUploading && setShowModal(false)}>
          <div className="bg-[#f0ebe3] rounded-3xl shadow-2xl w-full max-w-md overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            <div className="px-6 py-5 flex flex-col gap-4">
              {/* Storage Rules hint */}
              <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-amber-700 text-xs font-bold">
                ⚠️ تأكد من تفعيل Firebase Storage Rules:
                <code className="block mt-1 bg-white/60 px-2 py-1 rounded text-[10px] font-mono">
